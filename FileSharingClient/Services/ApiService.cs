            ...
            try
            {
                ...
                using (TcpClient client = new TcpClient(SERVER_IP, SERVER_PORT))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                    {
                        ...
                        Console.WriteLine($"[DEBUG][GetUserFilesAsync] Sending message: {message.Trim()}");
                        await writer.WriteLineAsync(message);
                        await writer.FlushAsync();
            ...
            try
            {
                ...
                using (TcpClient client = new TcpClient(SERVER_IP, SERVER_PORT))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                    {
                        ...
                        Console.WriteLine($"[DEBUG][GetUserFoldersAsync] Sending message: {message.Trim()}");
                        await writer.WriteLineAsync(message);
                        await writer.FlushAsync();
        ...
        public static async Task<List<FileItem>> GetSharedFilesAsync(int userId)
        {
            try
            {
                ...
                using (TcpClient client = new TcpClient(SERVER_IP, SERVER_PORT))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                    {
                        string message = $"GET_SHARED_FILES_DETAILED|{userId}\n";
                        Console.WriteLine($"[DEBUG][GetSharedFilesAsync] Sending message: {message.Trim()}");
                        await writer.WriteLineAsync(message);
                        await writer.FlushAsync();
            ...
            try
            {
                ...
                using (TcpClient client = new TcpClient(SERVER_IP, SERVER_PORT))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                    {
                        ...
                        Console.WriteLine($"[DEBUG][GetSharedFoldersAsync] Sending message: {message.Trim()}");
                        await writer.WriteLineAsync(message);
                        await writer.FlushAsync();
        ...
        public static async Task<bool> CreateFolderAsync(int userId, string folderName, int? parentFolderId = null)
        {
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                {
                    ...
                    Console.WriteLine($"[DEBUG] Sending message: {message.Trim()}");
                    await writer.WriteLineAsync(message);
